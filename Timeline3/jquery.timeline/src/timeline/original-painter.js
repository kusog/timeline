...
$.extend(Timeline.OriginalEventPainter.prototype, {
    ...
    addEventPaintListener: function(listener) {
        this._eventPaintListeners.push(listener);
    },

    removeEventPaintListener: function(listener) {
    ...
    _setOrthogonalOffset: function(metrics) {
    ...
    _prepareForPainting: function() {
        ...
        if (this._backLayer == null) {
            this._backLayer = this._band.createLayerDiv(0, "timeline-band-events").css("visibility", "hidden").show().get(0);
        ...
        this._lineLayer = band.createLayerDiv(110, "timeline-band-lines").get(0);
        this._lineLayer.style.display = "none";
    
        if (this._eventLayer != null) {
            band.removeLayerDiv(this._eventLayer);
        }
        this._eventLayer = band.createLayerDiv(115, "timeline-band-events").get(0);
    ...
    paintPreciseInstantEvent: function(evt, metrics, theme, highlightIndex) {
    ...
    paintImpreciseInstantEvent: function(evt, metrics, theme, highlightIndex) {
        var doc = this._timeline.getDocument();
        var text = evt.getText();
    ...
    paintPreciseDurationEvent: function(evt, metrics, theme, highlightIndex) {
        var doc = this._timeline.getDocument();
        var text = evt.getText();
    ...
    paintImpreciseDurationEvent: function(evt, metrics, theme, highlightIndex) {
        var doc = this._timeline.getDocument();
        var text = evt.getText();
    ...
    _paintEventIcon: function(evt, iconTrack, left, metrics, theme, tapeHeight) {
    ...
    _paintEventLabel: function(evt, text, left, top, width,
    ...
    _getLabelDivClassName: function(evt) {
        return this._getElClassName('timeline-event-label', evt, 'label');
    },

    _getElClassName: function(elClassName, evt, prefix) {
    ...
    _onClickInstantEvent: function(icon, domEvt, evt) {
    ...
    _onClickDurationEvent: function(target, domEvt, evt) {
        if ("pageX" in domEvt) {
            var x = domEvt.pageX;
            var y = domEvt.pageY;
        } else {
    ...
    _fireOnSelect: function(eventID) {
        for (var i = 0; i < this._onSelectListeners.length; i++) {
            this._onSelectListeners[i](eventID);
        }
    },

    _fireEventPaintListeners: function(op, evt, els) {
        for (var i = 0; i < this._eventPaintListeners.length; i++) {
